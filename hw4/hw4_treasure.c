...
//child sum+coins now we have
    int n_max = max(left_sum, right_sum) + root->coin;
    return n_max;
}
...
void path(nodeptr root, const int sum, int cur_sum, int *count)
{
    ...
    push(root->data);//path stack
    cur_sum = cur_sum+ root->coin;//update current max
    if(root->left == NULL && root->right == NULL && cur_sum == sum)//leaf and current sum=total sum
    {
    ...
    else if(root->left != NULL || root->right != NULL)//we have subtree
    {
        if(root->left != NULL)//traverse
